...
export const AddBusFastCallCar = (props) => {
    ...
    const getAllRoute = useCallback(async (useAPI = false) => {
        ...
        if (isUndefined(globalContextService.get("AddBusFastCallCarPage", "firstUseAPIgetAllRoute")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}busStationLiness/load?page=1&limit=99999`, // busStationLiness/load?page=1&limit=99999              
                ...
                .finally(() => {
                    ...
                    globalContextService.set("AddBusFastCallCarPage", "firstUseAPIgetAllRoute", false);
                    //#endregion
                });
            //#endregion
    ...
    const getStationOnRoute = useCallback(async (stationId) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        // if (isUndefined(globalContextService.get("AddBusFastCallCarPage", "firstUseAPIgetStationOnRoute")) || useAPI) {
        //#endregion
        ...
        fetch(`${APIUrl}busStationLiness/get?id=${stationId}`, // busStationLiness/get?id=6725790941894451200       
            ...
            .finally(() => {
                //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                globalContextService.set("AddBusFastCallCarPage", "firstUseAPIgetStationOnRoute", false);
                //#endregion
            });
        //#endregion
    ...
    const getAllStation = useCallback(async (useAPI = false) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        if (isUndefined(globalContextService.get("AddBusFastCallCarPage", "firstUseAPIgetAllStation")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}busStationss/load?page=1&limit=99999`, // busStationss/load?page=1&limit=99999          
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("AddBusFastCallCarPage", "firstUseAPIgetAllStation", false);
                    //#endregion
                });
            //#endregion
