...
const MobileMBase = (props) => {
    ...
    const { pages: { fastCallCar: { busFastCallCar: { rwd: { mobileM } } } } } = Theme;
    ...
    return (
        <>
            {data.filter( X => X.identity === "3" ).length === 0
                ?
                <>
                    {/* 無資料表單區容器 */}
                    < BasicContainer
                ...
                <>
                    ...
                    
                    {
                        <CardTable
                            ...
                            //sort
                            showHeader={false}
                            data={data.filter( X => X.identity === "3" )}
                            clickPage={(currentPage, pageSize) => {
                            }}
                        />
                    }

                    {data.filter( X => X.identity === "3").length < 11 
                        &&
                        <>
                            <Text
