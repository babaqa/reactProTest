...
import { ReactComponent as Convert } from '../../../../Assets/img/WhiteCallCarAgainPage/Convert.svg'
import { ReactComponent as StartToEnd } from '../../../../Assets/img/WhiteCallCarAgainPage/StartToEnd.svg'
import { ReactComponent as UpCircle } from '../../../../Assets/img/WhiteCallCarAgainPage/UpCircle.svg'
import { ReactComponent as End } from '../../../../Assets/img/WhiteCallCarAgainPage/End.svg'
import { ReactComponent as Start } from '../../../../Assets/img/WhiteCallCarAgainPage/Start.svg'
...
const LaptopBase = (props) => {
    ...
    const { pages: { callCarAgain: { whiteCallCarAgain: { rwd: { laptop } } } } } = Theme;
    ...
    const getCaseOrderAmtAPI = useCallback(() => {
        ...
        else if (valid(globalContextService.get("WhiteCallCarAgainPage", "TravelTime") ?? "", ["^.{1,}$"], ["請選擇乘車時間"])[1]) {
            validMsg = valid(globalContextService.get("WhiteCallCarAgainPage", "TravelTime") ?? "", ["^.{1,}$"], ["請選擇乘車時間"])[1]
        }
        ...
        if (validMsg === "") {
            ...
            props.GetCaseOrderAmtExecute({
                ...
                // ToAddrId:, // 不用丟
                ReservationDate: globalContextService.get("WhiteCallCarAgainPage", "TravelDate") + " " + globalContextService.get("WhiteCallCarAgainPage", "TravelTime"), // 預約日期+預約時間	如: "2020-11-25 17:45"
            })
        }
                    ...
                    >
                        <FormRow>
                            ...
                            {/*  乘車日期檢核 */}
                            {
                                ...
                                <>
                                    ...
                                    <DateTimePicker
                                        ...
                                        placeholder={""}
                                        value={
                                            (globalContextService.get("WhiteCallCarAgainPage", "TravelTime")) ?
                                                moment(globalContextService.get("WhiteCallCarAgainPage", "TravelTime"), "HH:mm")
                                                :
                                                null
                                        }
                                        onChange={(value, momentObj) => {
                            ...
                            <Radio
                                ...
                                // disable
                                topLabel={"願意共乘"}
                                value={globalContextService.get("WhiteCallCarAgainPage", "RideTogetherReview") ?? 1}
                                onChange={(e, value, onInitial) => {
                                    // console.log(value)
                                    globalContextService.set("WhiteCallCarAgainPage", "RideTogetherReview", value);
                                ...
                                onChange={(e, value, onInitial) => {
                                    if (!isEqual(value, globalContextService.get("WhiteCallCarAgainPage", "AccompanyCounts"))) {
                                        ...
                                        Array(preNum).fill(0).forEach((it, ind) => {
                                            ...
                                            globalContextService.remove("WhiteCallCarAgainPage", `TakerBrithday_${ind + 1}`)
                                        });
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            ...
                                            <DateTimePicker
                                                ...
                                                placeholder={""}
                                                value={
                                                    (globalContextService.get("WhiteCallCarAgainPage", `TakerBrithday_${index + 1}`)) ?
                                                        moment(globalContextService.get("WhiteCallCarAgainPage", `TakerBrithday_${index + 1}`), "YYYY-MM-DD HH:mm:ss")
                                                        :
                                                        null
                                                }
                                                onChange={(value, momentObj) => {
                                                    globalContextService.set("WhiteCallCarAgainPage", `TakerBrithday_${index + 1}`, value);
                                                }}
                            ...
                            onClick={() => {
                                    ...
                                    !(
                                        (Array(globalContextService.get("WhiteCallCarAgainPage", "AccompanyCounts")?.value)).fill(0)
                                            .map((item, index) => {
                                                ...
                                                return [
                                                    valid(globalContextService.get("WhiteCallCarAgainPage", `TakerName_${index + 1}`) ?? "", ["^.{1,}$"], [`請輸入搭車姓名${index + 1}`])[1],
                                                    valid(globalContextService.get("WhiteCallCarAgainPage", `TakerBrithday_${index + 1}`) ?? "", ["^.{1,}$"], [`請輸入搭車生日${index + 1}`])[1]
                                                ]
                                            }).flat().every(V => (V === null))
                                    )
                                ) {
                                ...
                                if (validMsg !== "") {
                                    ...
                                    props.AddOrderOfSelfPayUsersExecute({
                                        ...
                                        canShared: globalContextService.get("WhiteCallCarAgainPage", "RideTogetherReview") === 1 ? true : false, //願意共乘
                                        ...
                                        // id: ""	白牌預約訂單 id
                                        noticePhone: globalContextService.get("WhiteCallCarAgainPage", "SmsNumber"),	//畫面無此欄位
                                        orgId: "",//	畫面無此欄位
                                        ...
                                        remark: JSON.stringify((Array(globalContextService.get("WhiteCallCarAgainPage", "AccompanyCounts")?.value)).fill(0).map((item, index) => {
                                            return {
                                                ...
                                                birth: globalContextService.get("WhiteCallCarAgainPage", `TakerBrithday_${index + 1}`)
                                            }
                                        })),	//搭乘人員資訊陣列
                                        reserveDate: `${globalContextService.get("WhiteCallCarAgainPage", "TravelDate")} ${globalContextService.get("WhiteCallCarAgainPage", "TravelTime")}`,	//預約日期+ 預約時間
                                        selfPayUserId: props.CaseUserId, //白牌個案id
                                        status: 1,	//畫面無此欄位
                                        time: globalContextService.get("WhiteCallCarAgainPage", "TravelTime"), //預約時間
