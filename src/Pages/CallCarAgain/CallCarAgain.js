...
export const CallCarAgain = (props) => {
    ...
    const getUsers = useCallback(async (useAPI = false) => {
        ...
        if (isUndefined(globalContextService.get("CallCarAgainPage", "firstUseAPIgetUsers")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}Users/GetClient?id=${getParseItemLocalStorage("UserID")}`, //users/get
                ...
                .finally(() => {
                    ...
                    // globalContextService.set("CallCarAgainPage", "firstUseAPIgetUsers", false);
                    //#endregion
                });
            //#endregion
            ...
            fetch(`${APIUrl}categorys/load?page=1&limit=99999&TypeId=SYS_CAR`, //categorys/load?page=1&limit=20&TypeId=SYS_DRIVER_LICENSE
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    // globalContextService.set("CaseCallCarAgainComponentPage", "firstUseAPIgetAllCars", false);
                    //#endregion
                });
            //#endregion
    ...
    const getAllRoute = useCallback(async (useAPI = false) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        if (isUndefined(globalContextService.get("CallCarAgainPage", "firstUseAPIgetAllRoute")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}busStationLiness/load?page=1&limit=99999`, // busStationLiness/load?page=1&limit=99999              
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("CallCarAgainPage", "firstUseAPIgetAllRoute", false);
                    //#endregion
                });
            //#endregion
    ...
    const getAllStation = useCallback(async (useAPI = false) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        if (isUndefined(globalContextService.get("CallCarAgainPage", "firstUseAPIgetAllStation")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}busStationss/load?page=1&limit=99999`, // busStationss/load?page=1&limit=99999          
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("CallCarAgainPage", "firstUseAPIgetAllStation", false);
                    //#endregion
                });
            //#endregion
    ...
    const getStationOnRoute = useCallback(async (stationId) => {
        ...
        if (!isNil(stationId)) {
            ...
            fetch(`${APIUrl}busStationLiness/get?id=${stationId}`, // busStationLiness/get?id=6725790941894451200       
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("CallCarAgainPage", "firstUseAPIgetStationOnRoute", false);
                    //#endregion
                });
            //#endregion
