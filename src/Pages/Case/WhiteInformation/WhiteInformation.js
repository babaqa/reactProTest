...
export const WhiteInformation = (props) => {
    ...
    const controllGCS = (type, payload) => {
        switch (type) {
            case "return":
                ...
                globalContextService.remove("WhiteInformationPage", "Name");
                globalContextService.remove("WhiteInformationPage", "Birthday");
                globalContextService.remove("WhiteInformationPage", "Sex");
                globalContextService.remove("WhiteInformationPage", "Uid");
                globalContextService.remove("WhiteInformationPage", "Cellphone");
                // globalContextService.remove("WhiteInformationPage", "County");
                // globalContextService.remove("WhiteInformationPage", "District");
                globalContextService.remove("WhiteInformationPage", "Address"); // 合併成一個欄位
                globalContextService.remove("WhiteInformationPage", "DriverNote");
                globalContextService.remove("WhiteInformationPage", "ContactName");
                globalContextService.remove("WhiteInformationPage", "Relationship");
                globalContextService.remove("WhiteInformationPage", "ContactCellphone");
                globalContextService.remove("WhiteInformationPage", "ContactTelephone");
                globalContextService.remove("WhiteInformationPage", "Longitude0");
                globalContextService.remove("WhiteInformationPage", "Latitude0");
                ...
                if (payload === "API") {
                    globalContextService.remove("WhiteInformationPage", "CheckedRowKeys");
                    globalContextService.remove("WhiteInformationPage", "CheckedRowsData");
                }
                //#endregion
                break;
            ...
            case "backFromEditPage":
                ...
                globalContextService.remove("WhiteEditPage", "firstUseAPIgetSubOrgs");
    ...
    const getClient = useCallback(async (useAPI = false) => {
        ...
        if (isUndefined(globalContextService.get("WhiteInformationPage", "firstUseAPIgetClient")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}users/getclient?id=${urlParams.get("userId")}`, // users/getclient?id=6717458081668177920
                ...
                .finally((res) => {
                    ...
                    globalContextService.set("WhiteInformationPage", "firstUseAPIgetClient", false);
                    //#endregion
                });
            //#endregion
    ...
    const getWhiteUsers = useCallback(async (useAPI = false) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        if (isUndefined(globalContextService.get("WhiteInformationPage", "firstUseAPIgetCaseUsers")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}selfpayusers/get?id=${urlParams.get("caseUserId")}`, // selfpayusers/get?id=6718179154760081408  (以白牌 CaseUser table ID)
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("WhiteInformationPage", "firstUseAPIgetCaseUsers", false);
                    //#endregion
                });
            //#endregion
