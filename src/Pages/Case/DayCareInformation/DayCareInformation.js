...
export const DayCareInformation = (props) => {
    ...
    const controllGCS = (type, payload) => {
        switch (type) {
            ...
            case "backFromEditPage":
                ...
                globalContextService.remove("DayCareInformationPage", "firstUseAPIgetClient");
                globalContextService.remove("DayCareInformationPage", "firstUseAPIgetSubOrgs");
                globalContextService.remove("DayCareInformationPage", "firstUseAPIgetCaseUsers");
                //#endregion
                break;
            default:
                break;
    ...
    const getClient = useCallback(async (useAPI = false) => {
        ...
        if (isUndefined(globalContextService.get("DayCareInformationPage", "firstUseAPIgetClient")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}users/getclient?id=${urlParams.get("userId")}`, // users/getclient?id=6717458081668177920
                ...
                .finally((res) => {
                    ...
                    globalContextService.set("DayCareInformationPage", "firstUseAPIgetClient", false);
                    //#endregion
                });
            //#endregion
    ...
    const getCaseUsers = useCallback(async (useAPI = false) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        if (isUndefined(globalContextService.get("DayCareInformationPage", "firstUseAPIgetCaseUsers")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}caseusers/get?id=${urlParams.get("caseUserId")}`, // caseusers/get?id=6718179154760081408  (以長照 CaseUser table ID)
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("DayCareInformationPage", "firstUseAPIgetCaseUsers", false);
                    //#endregion
                });
            //#endregion
    ...
    const getManagerUnit = useCallback(async (useAPI = false) => {
        ...
        //#region 規避左側欄收合影響組件重新渲染 (渲染即觸發的每一個API都要有，useAPI (預設) = 0、globalContextService.set 第二個參數要隨API改變)
        if (isUndefined(globalContextService.get("DayCareInformationPage", "firstUseAPIgetSubOrgs")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}check/getSubOrgs?orgId=6721883683468910592`, // check/getSubOrgs?orgId=8ccf3297-8e45-43eb-8cc1-17476538b70a 
                ...
                .finally(() => {
                    //#region 規避左側欄收合影響組件重新渲染 (每一個API都要有)
                    globalContextService.set("DayCareInformationPage", "firstUseAPIgetSubOrgs", false);
                    //#endregion
                });
            //#endregion
