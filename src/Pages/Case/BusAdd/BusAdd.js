...
export const BusAdd = (props) => {
    ...
    const controllGCS = (type, payload) => {
        switch (type) {
            case "return":
                ...
                globalContextService.remove("BusAddPage", "Name");
                globalContextService.remove("BusAddPage", "Birthday");
                globalContextService.remove("BusAddPage", "Sex");
                globalContextService.remove("BusAddPage", "Uid");
                globalContextService.remove("BusAddPage", "Cellphone");
                globalContextService.remove("BusAddPage", "Telephone");
                globalContextService.remove("BusAddPage", "CardNumber");
                // globalContextService.remove("BusAddPage", "DriverNote");
                // globalContextService.remove("BusAddPage", "ContactName");
                // globalContextService.remove("BusAddPage", "Relationship");
                // globalContextService.remove("BusAddPage", "ContactCellphone");
                // globalContextService.remove("BusAddPage", "ContactTelephone");
                ...
                //#endregion
                if (payload === "API") {
                    globalContextService.remove("BusAddPage", "CheckedRowKeys");
                    globalContextService.remove("BusAddPage", "CheckedRowsData");
                }
                //#endregion
                break;
            case "Save":
    ...
    const getClient = useCallback(async (useAPI = false) => {
        ...
        if (isUndefined(globalContextService.get("BusAddPage", "firstUseAPIgetClient")) || useAPI) {
            //#endregion
            ...
            fetch(`${APIUrl}users/getclient?id=${urlParams.get("userId")}`, // users/getclient?id=6717458081668177920
                ...
                .finally(() => {
                    ...
                    globalContextService.set("BusAddPage", "firstUseAPIgetClient", false);
                    //#endregion
                });
            //#endregion
    ...
    //#region 新增幸福巴士身份 API 
    const addBusUser = useCallback(async (addOrUpdateRowdata) => {

        // console.log(updateRowdata)
        //#region 新增幸福巴士身份 API 
        fetch(`${APIUrl}busUsers/add`,
            {
                headers: {
                    "X-Token": getParseItemLocalStorage("Auth"),
                    "content-type": "application/json; charset=utf-8",
    ...
    const [AddBusUserExecute, AddBusUserPending] = useAsync(addBusUser, false);
    //#endregion 
        ...
        <>
            {
                ...
                <LaptopL
                    ...
                    AddBusUserExecute={AddBusUserExecute}
                    AddBusUserPending={AddBusUserPending}
